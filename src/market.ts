...
export class Market {
  ...
  maxPositionSz(mid: number): number {
    const equity = Math.min(this.account?.accountValue ?? 0, config.sleeveUsd);
    return lot((Math.max(equity, 0) * config.hlLeverage) / Math.max(mid, 1e-9), this.szDecimals);
  }
  ...
  async init() {
    ...
    if (this.wallet && this.ex) {
      ...
      await this.ex.updateLeverage({ asset: this.assetId, isCross: true, leverage: config.hlLeverage }).catch((e) => {
    ...
    console.log(`hyperliquid · ${this.pair} ${net} · ${this.coin} asset ${this.assetId} · szDecimals ${this.szDecimals} · ${config.dryRun ? "DRY RUN" : `wallet ${this.address}`}`);
    if (this.wallet) {
  ...
  noteFill(fill: FillPnlLike) {
    ...
    if (side && Number.isFinite(ts) && ts > 0 && Number.isFinite(price) && price > 0) {
      this.fillPrints.push({ ts, side, price, size: Number.isFinite(size) ? size : 0, dir: fillDir(fill.dir) });
    }
  }
  ...
  async send(side: Side, sizeSz: number, book: Book, cancel: number[], capped: boolean): Promise<Quote> {
    const price = quotePrice(side, book, this.szDecimals);
    const size = lot(sizeSz, this.szDecimals);
    if (size <= 0) {
      return { side, price: 0, size: 0, txHash: null, cancel, status: "reverted", orderId: null, capped };
    }
    ...
    if (!this.ex) {
      return { side, price: px, size, txHash: null, cancel, status: "sim", orderId: null, capped };
    }
    if (this.lastOid != null && this.lastSide === side && this.lastPrice === px) {
      return { side, price: px, size, txHash: null, cancel: [], status: "placed", orderId: this.lastOid, capped, unchanged: true };
    }
    ...
    try {
      if (this.lastOid != null && this.lastSide === side) {
        await this.ex.modify({ oid: this.lastOid, order });
        this.lastPrice = px;
        return { side, price: px, size, txHash: null, cancel: [], status: "placed", orderId: this.lastOid, capped };
      }
      ...
      if (st && typeof st === "object" && "resting" in st) {
        ...
        this.lastSide = side;
        this.lastPrice = px;
        return { side, price: px, size, txHash: null, cancel: oids, status: "placed", orderId: this.lastOid, capped };
      }
      if (st && typeof st === "object" && "filled" in st) {
        this.lastOid = null;
        this.lastSide = null;
        return { side, price: px, size, txHash: null, cancel: oids, status: "placed", orderId: st.filled.oid, capped };
      }
      this.lastOid = null;
      this.lastSide = null;
      return { side, price: px, size, txHash: null, cancel: oids, status: "reverted", orderId: null, capped };
    } catch (e) {
      ...
      return { side, price: px, size, txHash: null, cancel, status: "reverted", orderId: this.lastOid, capped };
    }
  }
}
